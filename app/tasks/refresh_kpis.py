...
async def refresh_kpis(target: date | None = None) -> dict:
    ...
    try:
        async with SessionLocal() as session:
            ...
            imp = {
                sid: float(v or 0) for sid, v in (await session.execute(q_imp)).all()
            }
            exp = {
                sid: float(v or 0) for sid, v in (await session.execute(q_exp)).all()
            }
            pwr = {
                sid: float(v or 0) for sid, v in (await session.execute(q_power)).all()
            }
