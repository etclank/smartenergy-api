    ...
    async with SessionLocal() as session:
        ...
        for mid, name in meters:
            ...
            res = await session.execute(
                select(func.max(EnergyImported.timestamp)).where(EnergyImported.meter_id == mid)
            )
            last_ts = res.scalar()
            ...
            for h in range(24 * days):
                ...
                session.add_all([
                    EnergyImported(timestamp=ts, measure_value=base + rnd.uniform(-0.3, 0.3), meter_id=mid),
                    EnergyExported(timestamp=ts, measure_value=max(0, base / 2 + rnd.uniform(-0.2, 0.2)), meter_id=mid),
                    EnergyReactive(
                        timestamp=ts,
            ...
            for d in range(days):
                day_date = (start_ts + timedelta(days=d)).date()
                session.add(MaxPower(timestamp=day_date, measure_value=rnd.uniform(3.0, 5.5), meter_id=mid))
                inserted += 1
