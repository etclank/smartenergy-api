...
SmartEnergy is deployed to the Cloud-Native Service Control Plane at `energy.platform.eoghanclancy.eu`. This document describes the validated portfolio deployment. It uses production-like controls while remaining deliberately single-node and non-HA.
...
- a production Kustomize package containing the API, worker, Beat, PostgreSQL, Redis, migration, private Services, network boundaries, Ingress, TLS Certificate, and middleware;
...
[`deploy/kubernetes/overlays/production`](../deploy/kubernetes/overlays/production/) is the complete application-owned production package, including PostgreSQL, Redis, and persistent claims. Project 1 owns the Namespace, ResourceQuota, AppProject, and Argo CD Application.
...
`Application/smartenergy` deploys the application repository's production Kustomize package and namespaced runtime resources. Project 1 owns that Argo CD Application and the private Prometheus integration.

The verified public image tag is `ghcr.io/etclank/smartenergy-api:5961af5ce36d3aed2fa81ac85da107627e9b2f35`. The production overlay pins registry digest `sha256:0d13398c342931d23d726b4db1903508272e45fefc27687f9cfe311798a4e363`. Its SPDX SBOM and SLSA provenance are attached in GHCR. This image source revision is intentionally distinct from the later deployment revision containing the reviewed overlay.
...
The baseline has a mechanical downgrade for disposable testing, but production rollback does not promise destructive downgrades. Future changes should use expand-and-contract compatibility or a forward fix.
...
Both services are intentionally non-HA. A PVC survives ordinary Pod replacement, and that recovery path has been validated, but local-path data remains tied to one node. PVC deletion or node loss loses the local data. Off-node backup and disaster recovery are intentionally outside the current portfolio scope. Redis AOF narrows the normal crash-loss window to roughly one second, but cannot guarantee exactly-once Celery delivery or prevent application-level duplicate work.

## Optional future backup and restore work

The repository retains an experimental [`CronJob/smartenergy-postgres-backup`](../deploy/kubernetes/operations/backup/cronjob.yaml) template and guarded restore helpers under `deploy/kubernetes/operations` for possible future work. They are excluded from the production Kustomize render, have not been made an operational recovery path, and are not required for the portfolio deployment.

If this work is resumed, an administrator would need to provision S3-compatible off-node storage and the optional `smartenergy-backup` Secret containing `BACKUP_ENDPOINT`, `BACKUP_BUCKET`, `BACKUP_ACCESS_KEY`, `BACKUP_SECRET_KEY`, and `BACKUP_REGION`. Retention, upload monitoring, clean restore, and recovery validation must all be completed before describing backup as operational.
...
The current production reference is `ghcr.io/etclank/smartenergy-api@sha256:0d13398c342931d23d726b4db1903508272e45fefc27687f9cfe311798a4e363`. GHCR visibility is public, so no image pull Secret is required.

The Python base is pinned as a readable tag plus multi-platform digest in `docker/Dockerfile`. To update it, inspect the current upstream manifest with `docker buildx imagetools inspect python:3.13-slim`, replace the verified digest in both the build argument and OCI base label, then rebuild and repeat the test and read-only smoke suites. Alembic uses Python PostgreSQL drivers and does not need `psql`; the main image therefore omits PostgreSQL clients. The optional backup and restore templates use separately pinned PostgreSQL and curl images.
...
The API requests `50m/128Mi` and limits `250m/256Mi`; the worker requests `50m/128Mi` and limits `300m/256Mi`; Beat requests `10m/96Mi` and limits `50m/128Mi`; PostgreSQL requests `75m/192Mi` and limits `300m/384Mi`; Redis requests `25m/48Mi` and limits `100m/96Mi`. Migration uses `25m/96Mi` requests and `150m/192Mi` limits. Beat's memory was raised after its disposable-cluster working set measured about 80 MiB. API uses a no-surge rollout, accepting single-replica downtime to limit temporary memory. Worker and singleton Beat use `Recreate`; a worker restart can redeliver work under late acknowledgements.

Application Pods run as UID/GID 10001; PostgreSQL and Redis run as UID/GID 999. Every Pod uses RuntimeDefault seccomp, drops all capabilities, disables privilege escalation and service-account token automount, and has a read-only root. Only bounded temporary paths and declared persistent data paths are writable. `/api/health/z` is startup/liveness; `/api/health/readyz` checks PostgreSQL readiness without making Redis a readiness dependency.

Stateful Services and StatefulSets use sync wave `-2`. Migration is an Argo CD `Sync` hook at wave `-1`, with a fixed name, source-revision annotation, and `BeforeHookCreation`. A failed migration blocks wave `0` application workloads and remains available for diagnosis. API, worker, and Beat use wave `0`; Ingress uses wave `1`. This preserves the order: externally provisioned Secrets, PostgreSQL and Redis, migration, application roles, then public routing.

Default-deny ingress and egress apply to all package Pods. DNS is limited to CoreDNS UDP/TCP 53. PostgreSQL accepts TCP 5432 only from API, worker, and migration identities. Redis accepts TCP 6379 only from API, worker, and Beat. Beat receives `DATABASE_URL` because Celery imports database-backed task modules, but policy still denies Beat-to-PostgreSQL traffic. Traefik may reach API TCP 8000, and Prometheus may reach metrics TCP 9090. No application role receives general internet egress. SendGrid and OTLP remain disabled.
...
The measured final package fits Project 1 quota. Steady state is `210m/1000m` CPU and `592Mi/1120Mi` memory. With the migration Job it is `235m/1150m` and `688Mi/1312Mi`. Five steady Pods plus one transient Job remain below the 12-Pod quota. PostgreSQL 5 GiB plus Redis 1 GiB uses two PVCs and 6 GiB, leaving one PVC and 6 GiB of quota.

Project 1 provisions the three Secret contracts, commit-pinned Argo Application, and Prometheus discovery edge. Argo CD applies the overlay through its manual sync policy.

## Capacity and rollout

The platform is a single approximately 4 GiB node. The validated deployment uses one replica per role, worker concurrency 1, explicit resources, and no-surge application rollouts. PostgreSQL, Redis, API, worker, Beat, ingress, and metrics were admitted incrementally with measurements after every checkpoint. The final observed node memory was approximately 75%, with no memory, disk, or PID pressure.
